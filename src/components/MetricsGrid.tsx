...
const telemetryStream = [
  { time: '09:00', ops: 3820, latency: 14.8, stage: 'Email & Notice Intake' },
  { time: '10:00', ops: 4790, latency: 14.1, stage: 'Permit & Inspection Check' },
  { time: '11:00', ops: 4210, latency: 13.9, stage: 'Contractor Follow-Up Agent' },
  { time: '12:00', ops: 5680, latency: 14.4, stage: 'COI & Draw Verification' },
  { time: '13:00', ops: 5120, latency: 13.8, stage: 'Monday.com Task Dispatch' },
  { time: '14:00', ops: 6450, latency: 13.2, stage: 'Underwriting Screening' },
  { time: '15:00', ops: 5890, latency: 13.5, stage: 'Draft Reply Generation' },
  { time: '16:00', ops: 6820, latency: 12.8, stage: 'Daily Executive Briefing' },
  { time: '17:00', ops: 6180, latency: 13.4, stage: 'Milestone Sign-Off' },
...
export function MetricsGrid() {
  ...
  const renderCardChart = (idx: number) => {
    ...
    if (idx === 0) {
      // Card 0: Automated operations growth curve (Stripe Blurple wavy gradient area)
      return (
                ...
                if (active && payload && payload.length) {
                  return (
                    <div className="rounded-[4px] bg-[var(--color-surface)] border border-[var(--color-border)] px-2.5 py-1 text-xs font-medium shadow-xs text-[var(--color-text-primary)]">
                      <span className="font-bold text-[#533AFD] dark:text-[#7A68FF]">${payload[0].value}</span> Operations Handled
                    </div>
                  );
                }
                return null;
